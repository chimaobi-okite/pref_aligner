...
from utils.data_utils import load_data, load_full_data
...
def aggregate_mcq_results(model_path: str, data_path: str, folder_path:str, pref_type: str, prompt_method: str):
    ...
    if data_path == "full":
        return aggregate_full_mcq_results(model_path, folder_path, pref_type, prompt_method)
...
def aggregate_full_mcq_results(model_path: str, folder_path:str, pref_type: str, prompt_method: str):
    """Aggregates all chunked CSV results, deletes chunk files, and calculates accuracy."""
    ...
    robustness = get_full_agg_robustness(merged_df)
    acc_diff = no_pref_acc - pref_acc
    print(f"lenght of final df is {len(merged_df)}")
    print(f"Robustness is {robustness}")
    print(f"Acc Diff is {acc_diff}")
    
    

    # Save final aggregated CSV
    final_output_path = f"{output_dir}/{model_name}_{robustness}_{acc_diff}_final.csv"
    merged_df.to_csv(final_output_path, index=False)
